...
class BudgetAdapterBehaviorTest {
    ...
    void summaryAdapterUnderTinyDegradesToMini() throws IOException {
        ...
        try {
            ...
            int exitCode = cli.execute("summary", "TestClass");
    ...
    @Test
    @DisplayName("B3/A5: ReadAdapter under TINY denies whole-class reads - verify actual denial")
    void readAdapterUnderTinyDeniesWholeClassReads() throws IOException {
        ByteArrayOutputStream errContent = new ByteArrayOutputStream();
        PrintStream originalErr = System.err;
        System.setErr(new PrintStream(errContent));
        ...
        try {
            ...
            CommandLine cli = new CommandLine(jsrcCmd);
            // Whole-class read (no method specified)
            int exitCode = cli.execute("read", "TestClass");
            
            // Should exit with BAD_USAGE (denied)
            assertEquals(ExitCode.BAD_USAGE, exitCode,
                "read TestClass (whole class) under TINY should be denied");
            
            String errOutput = errContent.toString();
            assertTrue(errOutput.contains("budget_denied"),
                "Should output budget_denied error");
            assertTrue(errOutput.contains("\"command\":\"read\""),
                "Should specify read command was denied");
    ...
    void readAdapterUnderTinyAllowsMethodReads() throws IOException {
        ...
        try {
            ...
            int exitCode = cli.execute("read", "TestClass.methodA");
    ...
    void describeCommandUnderTinyFiltersCommands() throws IOException {
        ...
        try {
            ...
            CommandLine cli = new CommandLine(jsrcCmd);
            int exitCode = cli.execute("describe");
            
            // Should succeed
            assertTrue(exitCode == ExitCode.OK || exitCode == 0,
                "describe should execute successfully");
